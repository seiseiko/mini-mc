...
ShaderProgram::ShaderProgram(OpenGLContext *context)
    : vertShader(), fragShader(), prog(),
      attrPos(-1), attrNor(-1), attrCol(-1),attrUV(-1),attrPow(-1),
      unifModel(-1), unifModelInvTr(-1), unifViewProj(-1), unifColor(-1),
      unifCamPos(-1),unifSampler2D(-1),unifTime(-1),
      unifShadowmap(-1),unifDepthMVP(-1),
      context(context)
{}
...
void ShaderProgram::create(const char *vertfile, const char *fragfile)
{
    ...
    attrPow = context->glGetAttribLocation(prog,"vs_Power");
...
void ShaderProgram::setDepthMVP(const glm::mat4 &dp)
{
...
void ShaderProgram::draw(Drawable &d, int textureSlot = 0,bool transparent =false)
{
    useMe();

    if(unifSampler2D!=-1)
    {
        context->glUniform1i(unifSampler2D, /*GL_TEXTURE*/textureSlot + 2);
    }
    if(unifShadowmap != -1)
     {
    ...
    int stride= 3 * sizeof(glm::vec4);

    if (attrPos != -1 && d.bindVert()) {
        context->glEnableVertexAttribArray(attrPos);
        context->glVertexAttribPointer(attrPos, 4, GL_FLOAT, false, stride, (void*)0);
    }

    if (attrNor != -1 && d.bindVert()) {
        context->glEnableVertexAttribArray(attrNor);
        context->glVertexAttribPointer(attrNor, 4, GL_FLOAT, false, stride, (void*)(sizeof(glm::vec4)));
    }

    if (attrCol != -1 && d.bindVert()) {
        context->glEnableVertexAttribArray(attrCol);
        context->glVertexAttribPointer(attrCol, 4, GL_FLOAT, false, stride, (void*)(2*sizeof(glm::vec4)));
    }
    ...
    if (attrPow != -1 && d.bindPow()) {
        context->glEnableVertexAttribArray(attrPow);
        context->glVertexAttribPointer(attrPow, 1, GL_FLOAT, false, 0, NULL);
    }
    ...
    if (attrPow != -1) context->glDisableVertexAttribArray(attrPow);
